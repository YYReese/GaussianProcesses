...
In this part, we discuss the performance of the model under three criteria. 

First, let the initial design to be -4. We are going to investigate which criterion will give less SE and DS scores. 
...
This is the plot of the averaged of SE scores of three models.
```{r,echo=FALSE}
ggplot(table1)+
  geom_line(aes(x=max_iterations, y=SE_MMSE, col="SE (MMSE)")) +
  geom_line(aes(x=max_iterations, y=SE_IMSE, col="SE (IMSE)")) +
  geom_line(aes(x=max_iterations, y=SE_IMDS, col="SE (IMDS)")) +
  xlab("Number of observations") +
  ylab("Averaged SE scores") +
  theme_bw() 
```

In general, averaged SE scores decreases as observations increases. We can see that IMSE gives the least averaged SE score while IMSE gives the largest. However, as the number of observations increases, the differences decreases.Notice that after 20 observations, the differences of the averaged SE between the three has already been negligible. It can therefore be inferred that the difference of the total variance will converge as max iteration goes up.



The plot of the averaged DS scores.
```{r,echo=FALSE}
ggplot(table1)+
  geom_line(aes(x=max_iterations, y=DS_MMSE, col="DS (MMSE)")) +
  geom_line(aes(x=max_iterations, y=DS_IMSE, col="DS (IMSE)")) +
  geom_line(aes(x=max_iterations, y=DS_IMDS, col="DS (IMDS)")) +
  xlab("Number of observations") +
  ylab("Averaged DS scores") +
  theme_bw() 
```
As for the averaged DS scores, 


The plot of time cost
```{r,echo=FALSE}
ggplot(table1)+
  geom_line(aes(x=max_iterations, y=time_cost_MMSE, col="Time cost (MMSE)")) +
  geom_line(aes(x=max_iterations, y=time_cost_IMSE, col="Time cost (IMSE)")) +
  geom_line(aes(x=max_iterations, y=time_cost_IMDS, col="Time cost (IMDS)")) +
  xlab("Iterations") +
  ylab("Time cost") +
  theme_bw() 
```
...
ggplot(table1) +
  geom_line(aes(x=max_iterations, y=variances_1, col="Variances (MMSE)")) +
  geom_line(aes(x=max_iterations, y=variances_2, col="Variances (IMSE)")) +
  geom_line(aes(x=max_iterations, y=time_cost_1, col="Time cost (MMSE)")) +
  geom_line(aes(x=max_iterations, y=time_cost_2, col="Time cost (IMSE)")) +
  xlab("iterations") +
...
var_dat1$iterations <- factor(var_dat1$iterations,level=1:10)
```
```{r,echo=FALSE}
var_dat1 %>%
  ggplot() +
...
var_dat2$iterations <- factor(var_dat2$iterations,level=1:10)
```
```{r,echo=FALSE}
var_dat2 %>%
  ggplot() +
...
```{r, echo=FALSE}
var_dat3 <- readRDS(file = "data/var_dat3.rds")
var_dat3$iterations <- factor(var_dat3$iterations, levels=1:10)
var_dat3 %>% 
  ggplot() +
  geom_line(aes(x,var)) +
  xlab("x") +
...
```{r, echo=FALSE}
var_dat4 <- readRDS(file = "data/var_dat4.rds")
var_dat4$iterations <- factor(var_dat4$iterations, levels=1:10)
var_dat4 %>%
  ggplot() +
  geom_line(aes(x,var)) +
  xlab("x") +
...
```
```{r}
var_datDS$iterations <- factor(var_datDS$iterations, levels=1:10)


ggplot() +
  geom_line(aes(x,var, col = "(MMSE)"), data = var_dat1) +
  geom_line(aes(x,var, col = "(IMSE)"), data = var_dat2) +
  geom_line(aes(x,var, col = "(IMDS)"), data = var_datDS) +
  xlab("x") +
  ylab("variance") +
  ggtitle("Prediction variance plot") +
  facet_grid(rows = vars(iterations),scales = "free_y")
```
...
ggplot(seq) +
  ...
  ylab("Overall variance") +
  xlab("Number of initial observations from uniform space-filling designs")
```
...
f_km_c1 <- seq_DoE(x,y1,initial_design, criterion_1,Matern_k) # Mat Mat c1

f_ke_c1 <- seq_DoE(x,y1,initial_design, criterion_1,Exp_k) # Mat Exp c1

f_km_c2 <- seq_DoE(x,y1,initial_design, criterion_2,Matern_k) # Mat Mat c2

f_ke_c2 <- seq_DoE(x,y1,initial_design, criterion_2,Exp_k) # Mat Exp c2

f_km_c3 <- seq_DoE(x,y1,initial_design, criterion_DS,Matern_k) # Mat Mat c3

f_ke_c3 <- seq_DoE(x,y1,initial_design, criterion_DS,Exp_k) # Mat Exp c3

g_km_c1 <- seq_DoE(x,y2,initial_design, criterion_1,Matern_k) #Exp Mat c1

g_ke_c1 <- seq_DoE(x,y2,initial_design, criterion_1,Exp_k) # Exp Exp c1

g_km_c2 <- seq_DoE(x,y2,initial_design, criterion_2,Matern_k) #Exp Mat c2

g_ke_c2 <- seq_DoE(x,y2,initial_design, criterion_2,Exp_k) # Exp Exp c2

g_km_c3 <- seq_DoE(x,y2,initial_design, criterion_DS,Matern_k) #Exp Mat c3
 
g_ke_c3 <- seq_DoE(x,y2,initial_design, criterion_DS,Exp_k) # Exp Exp c3
...
ggplot(f_km_c2)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[f_km_c2$obs],y=y1[f_km_c2$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
...
ggplot(f_ke_c2)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[f_ke_2$obs],y=y1[f_ke_c2$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
...
ggplot(g_km_c2)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[g_km_c2$obs],y=y2[g_km_c2$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
...
ggplot(g_ke_c2)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[g_ke_c2$obs],y=y2[g_ke_c2$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
...
```{r,echo=FALSE}
fg_data <- data.frame(f=c(sum((f_ke_c2$y_hat-y1)^2)*dx,sum((f_km_c2$y_hat-y1)^2)*dx),
           g=c(sum((g_ke_c2$y_hat-y2)^2)*dx,sum((g_km_c2$y_hat-y2)^2)*dx))
...
```{r,echo=FALSE}
c_data <- data.frame(f_Exp=c(sum((f_ke_c1$y_hat-y1)^2)*dx,
                             sum((f_ke_c2$y_hat-y1)^2)*dx,
                             sum((f_ke_c3$y_hat-y1)^2)*dx),
                     f_Mat=c(sum((f_km_c1$y_hat-y1)^2)*dx,
                             sum((f_km_c2$y_hat-y1)^2)*dx,
                             sum((f_km_c3$y_hat-y1)^2)*dx),
                     g_Exp=c(sum((g_ke_c1$y_hat-y2)^2)*dx,
                             sum((g_ke_c2$y_hat-y2)^2)*dx,
                             sum((g_ke_c3$y_hat-y2)^2)*dx),
                     g_Mat=c(sum((g_km_c1$y_hat-y2)^2)*dx,
                             sum((g_km_c2$y_hat-y2)^2)*dx,
                             sum((g_km_c3$y_hat-y2)^2)*dx))

rownames(c_data) <- c("MMSE","IMSE","IMDS")
...
ggplot(f_ke_c2)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[f_ke_c2$obs],y=y1[f_ke_c2$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
  ggtitle("Fitted GP model (exp_kernel) for g")
...
ggplot(f_ke_c3)+
  geom_ribbon(aes(x = x, ymin = y_hat-1.97*sqrt(var), ymax = y_hat+1.97*sqrt(var)),
              alpha = 0.2) +
  geom_line(aes(x = x, y = y_hat), linetype = "dashed") +
  geom_point(data = data.frame(x=x[f_ke_c3$obs],y=y1[f_ke_c3$obs]),
             aes(x = x, y = y),size = 2) +
  theme_bw() +
  geom_line(aes(x=x,y=y), colour= "red")+
  ggtitle("Fitted GP model (exp_kernel) for g")
